...
class ReleaseBuilder {
  ...
  async build() {
    ...
    if (this.release.state == ReleaseState.succeeded) {
      logger.info(
        `[id=${this.release.id}] skip for state ${this.release.state}.`
      );
      return;
    }
    // Handle building state.
    if (
      this.release.state == ReleaseState.failed &&
      this.release.reason == ReleaseReason.timeout
    )
      ...
      this.release.state == ReleaseState.pending ||
      this.release.state == ReleaseState.failed
    )
    ...
    if (build === null) {
      ...
      throw new Error(
        `[id=${this.release.id}] [build_id=${this.release.build_id}] build pipelines timeout.`
      );
    } else if (
      ...
      logger.info(
        `[id=${this.release.id}] [build_id=${this.release.build_id}] build pipelines succeeded.`
      );
    } else {
      // Pipelines failed.
      ...
      if (
        reason != ReleaseReason.badGateway &&
        reason != ReleaseReason.serverError
      ) {
        // Acceptable failure reason, just log it.
        logger.error(
          `[id=${this.release.id}] [build_id=${this.release.build_id}] build pipelines failed, status ${statusName}, result ${resultName}, reason ${reason}`
        );
      } else {
        // Other failure reason, raise error to retry.
        throw new Error(
          `[id=${this.release.id}] [build_id=${this.release.build_id}] build pipelines failed, status ${statusName}, result ${resultName}`
        );
      }
    }
  }
  ...
  getReasonFromPublishLog(text) {
    if (text.includes("EPUBLISHCONFLICT")) return ReleaseReason.publishConflict;
    else if (
      text.includes("ENOENT") &&
      text.includes("error path package.json")
    )
      return ReleaseReason.nonPackage;
    else if (text.includes("error code E502")) return ReleaseReason.badGateway;
    else if (text.includes("error code E500")) return ReleaseReason.serverError;
    return "";
  }
  ...
  async checkBuildPipelines() {
    logger.info(
      `[id=${this.release.id}] [build_id=${this.release.build_id}] check build pipelines`
    );
    for (let i = 0; i < config.azureDevops.check.retries; i++) {
      let build = await this.buildApi.getBuild(
        config.azureDevops.project,
        this.release.build_id
      );
      let statusName = BuildStatusEnum.getKeyOrThrow(build.status);
      let resultName =
        typeof build.result === "undefined"
          ...
          : BuildResultEnum.getKeyOrThrow(build.result);
      logger.info(
        `[id=${this.release.id}] [build_id=${this.release.build_id}] status ${statusName}, result ${resultName}, retries ${i}`
      );
      if (
        build.status == BuildStatus.Completed ||
...
const getBuildName = function(releaseId, packageName, packageVer) {
  ...
  if (buildName.startsWith("@")) buildName = buildName.substr(1);
