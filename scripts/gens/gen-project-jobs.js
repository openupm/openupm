...
const genProjectJobs = async function(ids) {
  let lookup = null;
  if (ids && ids.length) lookup = query => query.whereIn("id", ids);
  ...
  for (let project of projects) {
    ...
    if (queue.isJobFailedCompletely(job)) {
      ...
      logger.info(
        `[id=${project.id}] job existed. status=${job.status}, retries-left=${job.options.retries}.`
      );
      continue;
    }
    job = await queue
      ...
      .save();
    if (job.id) logger.info(`[id=${project.id}] job added ${jobId}.`);
  }
};
